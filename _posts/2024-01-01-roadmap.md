...
- **Maintenance**: GeoServer was started in 2001 by a non-profit technology incubator. Subsequent years then saw the project supported by larger companies with investors and venture capital. This support is no longer available - and without this cushion we must rely on our community to play a greater role in sharing ongoing maintenance activities.
...
* **Migrate to ImageN 1.0**
  ...
  1. First working with Boundless to outline the creation of a new library ($150k) after assessing alternatives in the Java ecosystem (nothing matched JAI on-demand capabilities required for geospatial content).
  ...
  3. Jody has worked on this project as a background activity when unemployed and the source code now compiles in a modern environment with documentation that has been migrated to markdown (consider that at $25k savings)
