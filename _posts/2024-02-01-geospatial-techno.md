...
## Workspaces
Geoserver's Workspace serves as a means to group and organize similar layers together. It enables you to associate multiple layers with a single workspace. As you add more layers, the importance of workspaces increases, as they become essential for better organization and easier sorting of layer previews. Generally, a workspace is created for each project, along with its corresponding stores and layers.

#### Add a workspace
To create a new workspace, navigate to the "Data" section and "Workspaces" link. Click on "Add new workspace". Now you have to enter a workspace "Name" and "Namespace URI".
- Workspace Name: It is an identifier describing your project. It must not exceed ten characters or contain spaces.
...
- Check Default Workspace to assign this as your default workspace.
- Sometimes the same feature type needs to be published multiple times with a different mapping and with the same name. This can be done in GeoServer using "Isolated Workspaces" functionality. Click the "Submit" to save your new workspace.

#### Edit a workspace
To view or edit a workspace, click the Workspace name, then a workspace configuration page will be displayed.
- Check the "Enabled" checkbox to have separate metadata at the workspace level and global service for this workspace.
- Use the checkbox located next to each service to override the Global Settings for the associated service. Once enabled, clicking on the service link will open the settings page for the service, allowing default values for the service title, abstract, and other details to be supplied.
...
## Stores
GeoServer's Stores serve as intermediaries between the server and the data sources where your spatial data resides. They provide a mechanism for GeoServer to connect to various data repositories, including file systems, databases, and cloud storage services. Each store represents a unique data source and has its configuration settings.

#### Add a store
To add a Store, click "Stores" from the "Data" section. Click the "Add new store" button. You will be prompted to choose a data source. GeoServer supports several different data formats, but they are classified into three types: Vector data, Raster data, and Cascaded Services.
...
- Enter the Data Source Name. Make sure the Enabled option is checked. Otherwise, access to the store along with all the datasets defined, will be disabled for it.
- In the "Shapefile location", click on the "Browse" link to define the location of the shapefile.
- DBF charset is used to decode strings from the dbf file, so it's very important to select an appropriate DBF character set.
When finished, click on the Save button. Now it will automatically redirect to the "Add New Layer page", which will be completely described in the Layer section. Next, we will explain how to edit and remove the store.
...
#### Remove a store
To remove a Store, click the checkbox next to the store. Multiple stores can be selected, or all can be selected by clicking the checkbox in the header. Click the "Remove selected stores" button. You will be asked to confirm the removal of the configuration for the store(s) and all resources defined under them. Clicking OK removes the selected Store(s), and returns to the Stores page.

## Layers
From the administration interface, go to the "Data" section and click on the "Layers" section. 
...
#### Add a layer
Clicking the "Add a new layer" button brings up a New Layer Chooser panel. The menu displays all currently enabled stores. If you want to add a new layer for a published resource, click Publish Again. Note that when republishing the name of the new layer may have to be modified to avoid conflict with an existing layer.
...
The "Bounding Box" determines the extent of the data within a layer. It includes two items: "Native Bounding Box" and "Lat/Lon Bounding Box". Generate the bounds for the layer by clicking the "Compute from data" and "Compute from native bounds" button in the "Bounding Boxes" section.

Vector layers have a list of the "Feature Type Details". These include the "Property" and "Type" of a data source. Remember that, if you want to change your data by ArcGIS or QGIS, like add or remove features or fields, or edit the attribute table contents, there is no need to create a layer again in the GeoServer, just click On "Reload feature type", so your layer will be updated.

Remember that GeoServer, by default, publishes all the features that are currently available in the layer. However, if you wish to limit the features to a specific subset, you can achieve this by specifying a CQL filter in the configuration. Upon completing the layer configuration, finalize the process by clicking the "Save" button. This action will create the layer based on the specifications you have provided.

#### Edit a layer
To view or edit a layer, click the "Layer Name" from the Layer section. A layer configuration page will be displayed. After your configuration is modified, press the Save button.

#### Remove a layer
To remove a layer, click the checkbox next to the layer. Multiple layers can be selected, or all can be selected by clicking the checkbox in the header. Click the "Remove selected layers" button. You will be asked to confirm the removal of the configuration for the layer(s) and all resources defined under them. Clicking OK removes the selected layer(s), and returns to the Layers page.